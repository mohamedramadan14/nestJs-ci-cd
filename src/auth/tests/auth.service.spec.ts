...
describe("AuthService", () => {
    ...
    beforeEach(async () => {
        const module: TestingModule = await Test.createTestingModule({
            providers: [AuthService, JwtService, { provide: getModelToken(User.name), useValue: mockedAuthService }],
        }).compile();
    ...
    describe("Testing signUp", () => {
        it("It should register a new user and return a token", async () => {
            ...
            jest.spyOn(model, "create").mockImplementationOnce(() => Promise.resolve(mockedUser));
            ...
            expect(model.create).toHaveBeenCalledWith({ ...signUpDto, password: hashedPassword } as SignUpDto);
        ...
        it("It should throw an error if the user already exists", async () => {
            ...
            await expect(authService.signUp(signUpDto as SignUpDto)).rejects.toThrow(ConflictException);
            expect(model.create).toHaveBeenCalledWith({ ...signUpDto, password: hashedPassword } as SignUpDto);
        })
    })
    ...
    describe("Testing login", () => {
        it("It should login a user and return a token", async () => {
            ...
            expect(result).toEqual({ token });
            expect(model.findOne).toHaveBeenCalledWith({ email: mockedLoginDto.email });
            expect(bcrypt.compare).toHaveBeenCalledWith(mockedLoginDto.password, mockedUser?.password);
            expect(jwtService.sign).toHaveBeenCalledWith({ id: mockedUser._id });
        })
        ...
        it("It should throw an error if the user does not exist", async () => {
            ...
            await expect(authService.login(mockedLoginDto)).rejects.toThrow(UnauthorizedException);
            expect(model.findOne).toHaveBeenCalledWith({ email: mockedLoginDto.email });
        })
        ...
        it("It should throw an error if the password is incorrect", async () => {
            ...
            await expect(authService.login({ ...mockedLoginDto, password: mockedLoginDto.password + "1" })).rejects.toThrow(UnauthorizedException);
            expect(model.findOne).toHaveBeenCalledWith({ email: mockedLoginDto.email });
            expect(bcrypt.compare).toHaveBeenCalledWith(mockedLoginDto.password + "1", mockedUser?.password); // Adding "1" to the password to make it incorrect
        })
    })
})